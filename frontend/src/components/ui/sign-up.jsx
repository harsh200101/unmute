...
import {
  ArrowRight, Mail, User, Lock, Eye, EyeOff, ArrowLeft,
  X, AlertCircle, PartyPopper,
} from 'lucide-react';
...
/* ============================================================================
 * Multi-step glass-style sign-up shell.
 * Wizard order: name → email → password → confirmPassword.
 * The upstream snippet has no name step; we added one because the project's
 * `register({ full_name, email, password })` API requires it.
 *
 * Props
 *   logo, brandName              — branding shown top-left
 *   onRegister({full_name,...})  — must throw on failure; resolved => success
 *   onGoogleSignIn()             — fires the Google OAuth handoff
 *   onSuccess()                  — called once the success modal has landed
 * ==========================================================================*/
...
// ----------------------------- TextLoop ------------------------------------
function TextLoop({
  children, className, interval = 1.5,
...
function BlurFade({
  ...
  return (
    <motion.div
      ...
      exit="hidden" variants={combinedVariants}
...
// MAIN
// ============================================================================
export const SignUpAuthComponent = ({
  logo = null,
  brandName = 'unmute',
  onRegister,
  onGoogleSignIn,
  onSuccess,
  onSignIn,           // tapped on the "Already have an account? Sign in" link
}) => {
  ...
  const [authStep, setAuthStep]                       = useState('name'); // name → email → password → confirmPassword
  ...
  const handleFinalSubmit = async (e) => {
    ...
    try {
      ...
      setModalErrorMessage(ex?.response?.data?.error || ex?.message || 'Registration failed.');
      setModalStatus('error');
    }
  };
  ...
  const Modal = () => (
    <AnimatePresence>
      {modalStatus !== 'closed' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/50 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-card/90 border-4 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2 text-center">
            {(modalStatus === 'error' || modalStatus === 'success') && (
              <button onClick={closeModal} className="absolute top-2 right-2 p-1 text-muted-foreground hover:text-foreground transition-colors" aria-label="Close">
                <X className="w-5 h-5" />
              </button>
            )}
            {modalStatus === 'error' && (
            ...
            {modalStatus === 'loading' && (
              <TextLoop interval={1.5} stopOnEnd>
                {[
                  { message: 'Creating your account…', icon: <LumaSpin size={56} /> },
                  { message: 'Setting things up…',      icon: <LumaSpin size={56} /> },
                ].map((step, i) => (
            ...
            {modalStatus === 'success' && (
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-lg font-medium text-foreground">Welcome to unmute!</p>
              </div>
            )}
          </motion.div>
        </motion.div>
      ...
      <div className={cn('flex w-full flex-1 h-full items-center justify-center bg-card', 'relative overflow-hidden')}>
        ...
        <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
          ...
          <AnimatePresence mode="wait">
            {authStep === 'name' && (
              <motion.div key="name-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="w-full flex flex-col items-center gap-4">
                <BlurFade delay={0.25 * 1} className="w-full">
                  <div className="text-center">
                    <p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">Join unmute</p>
                  </div>
                </BlurFade>
                <BlurFade delay={0.25 * 2}><p className="text-sm font-medium text-muted-foreground">Let's start with your name.</p></BlurFade>
                {onGoogleSignIn && (
                  <>
            ...
            {authStep === 'email' && (
              <motion.div key="email-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="w-full flex flex-col items-center text-center gap-4">
                <BlurFade delay={0} className="w-full">
                  <div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">What's your email?</p></div>
                </BlurFade>
                <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">We'll send a verification link there.</p></BlurFade>
              </motion.div>
            )}
            {authStep === 'password' && (
              <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: 'easeOut' }} className="w-full flex flex-col items-center text-center gap-4">
                <BlurFade delay={0} className="w-full">
                  <div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">Create a password</p></div>
                </BlurFade>
                <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">At least 8 characters.</p></BlurFade>
              </motion.div>
            )}
