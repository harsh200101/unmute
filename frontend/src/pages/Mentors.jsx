...
const Mentors = () => {
  ...
  // State management for browse tab - Initialize arrays properly
  ...
  // State management for availability tab
  ...
  }, [activeTab, searchTerm, categoryFilter, languageFilter, ratingFilter, badgeLevelFilter, sortOrder, page, setSearchParams]);

  // Fetch mentors when dependencies change (only for browse tab)
  useEffect(() => {
    if (activeTab === 'browse') {
      fetchMentors();
    }
  ...
  // Fetch availability when date changes (only for availability tab)
  useEffect(() => {
    if (activeTab === 'availability') {
      fetchAvailability(selectedDate);
    }
  }, [fetchAvailability, selectedDate, activeTab]);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {mentors.map((mentor) => (
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center text-white text-xl font-bold flex-shrink-0 overflow-hidden">
                          {mentor.avatarUrl ? (
                ...
                {Array.from({ length: 8 }, (_, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`text-xs ${isSelected ? 'text-indigo-100' : 'text-gray-500'}`}>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900">
                  Available Mentors on {new Date(selectedDate).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                ...
                {availabilityLoading && <LoadingSpinner size="sm" />}
              </div>
              ...
              {availabilityLoading ? (
                ...
                <div className="grid gap-4">
                  {['10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00', '18:00', '19:00', '20:00', '21:00'].map((timeSlot) => {
                    const mentors = availabilityData[timeSlot] || [];

                    return (
                      <div key={timeSlot} className="border border-gray-200 rounded-xl p-4">
                        ...
                        {mentors.length === 0 ? (
                          <div className="text-sm text-gray-500 italic">
                            No mentors available during this time slot
                          </div>
                        ) : (
                          <div className="flex flex-wrap gap-2">
                            {mentors.map((mentor) => (
                              <button
                                key={mentor.id}
                                onClick={() => handleMentorClick(mentor.id)}
                                className="inline-flex items-center gap-2 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg transition-colors text-sm"
                              >
                                <div className="w-6 h-6 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                  {mentor.avatarUrl ? (
                                    <img src={mentor.avatarUrl} alt={mentor.name} className="w-full h-full object-cover rounded-full" />
                                  ) : (
                                ...
                                {mentor.badgeLevel && (
                                  <span className={`text-xs px-1.5 py-0.5 rounded ${
                                    ...
                                    mentor.badgeLevel === 'platinum' ? 'bg-purple-100 text-purple-800' :
                                    mentor.badgeLevel === 'diamond' ? 'bg-blue-100 text-blue-800' :
